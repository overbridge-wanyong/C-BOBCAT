...
class PPO:
    def __init__(self, state_dim, action_dim, lr, betas, K_epochs, eps_clip, lamda):
        self.lr = lr
        self.betas = betas
    ...
    def update(self, memory):
        ...
        for _ in range(self.K_epochs):
            ...
            loss = -torch.min(surr1, surr2) + 0.5 * \
                self.MseLoss(state_values, rewards) - self.lamda * dist_entropy
...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, gumbel=False, tau=1, n_latent_var=256):
        super().__init__()
        # actor
        self.gumbel = gumbel
        self.tau = tau
    ...
    def forward(self, state, action_mask):
        ...
        train_mask, actions = hard_gumbel_sample(logits,tau=self.tau) if self.gumbel else  hard_sample(logits)
        return train_mask, actions, entropy


class StraightThrough:
    def __init__(self, state_dim, action_dim, lr, betas, gumbel= False, tau= 1):
        self.lr = lr
        self.betas = betas
        self.policy = Actor(state_dim, action_dim,gumbel, tau).to(device)
        self.optimizer = torch.optim.Adam(
            self.policy.parameters(), lr=lr, betas=betas)
